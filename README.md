...
A fully serverless REST API built on AWS, with no servers to manage. Create, read, update, and delete tasks via HTTP — all running on Lambda, API Gateway, and DynamoDB.
...
**4. Create and deploy API Gateway:**
```bash
aws apigateway create-rest-api --name todo-api --endpoint-configuration types=REGIONAL
# Follow setup steps to connect API Gateway to Lambda
```
...
- Infrastructure deployment via AWS CLI
